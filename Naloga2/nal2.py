...
def LevyFlight(Nsets, Nruns, Nsteps, Lmin, mu, plot):
    dt = 1 #time per step
    ts = [t for t in range(0, (Nsteps+1) * dt, dt)]

    mads = np.ones((Nsets,Nsteps+1))
    ...
    for i in tqdm(range(Nsets), desc="LevyFlight Progress", ):
        ...
        rss = np.sqrt(np.square(xss) + np.square(yss)) ##### Razdalja od izhodišča
...
def LevyWalk(Nsets, Nruns, Nsteps, Lmin, mu, plot):
    velocity = 1
    Ntimes = Nsteps + 1
    ts = np.linspace(0,Nsteps * Lmin / velocity, Ntimes)
    ...
    for i in tqdm(range(Nsets), desc="LevyWalk Progress"):
        ...
        #Convert step index to time index
        xts = np.zeros((Nruns, Ntimes))
        yts = np.zeros((Nruns, Ntimes))
        ...
        for k in range(len(xss[:,0])):
            ...
            for j in range(Nsteps):
                #print(f"Time conversion {i} {k} {j}")
                current_distance = 0

                while(sumL >= current_distance):
                    try:
                        current_distance = ts[timeIndex] * velocity
                    except IndexError:
                        break

                    if sumL == current_distance:
                        xt.append(xs[j]); yt.append(ys[j])
                        timeIndex += 1
                    
                    elif sumL + Ls[j] > current_distance:
                        t1 = sumL / velocity
                        t2 = t1 + Ls[j] / velocity
                        xt.append(xs[j] + (xs[j+1] - xs[j]) * (ts[timeIndex]-t1) / (t2-t1))
                        yt.append(ys[j] + (ys[j+1] - ys[j]) * (ts[timeIndex]-t1) / (t2-t1))
                        timeIndex += 1
            ...
            plt.scatter(xt,yt,s=10,color="black")
            plt.show()
            """
            xts[k,:] = np.array(xt)
            yts[k,:] = np.array(yt)
            #bar.next()
        #Continue as before, just with time based index this time
...
#LevyFlight(Nsets=10, Nruns=10000, Nsteps=1000, Lmin=1, mu=2, plot = True)
#LevyWalk(Nsets=10, Nruns=10000, Nsteps=1000, Lmin=1, mu=2, plot = True)
