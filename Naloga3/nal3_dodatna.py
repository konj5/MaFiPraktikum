...
def getNondiagonalHamiltonian(N:int, λ:float, q:str) -> np.array:
    ...
    def Hij(i,j):
        return (1/2*(i+1/2) - 3/2 * expected_q2(i,j) + 1/10*q4(i,j)) if i == j else (-3/2 * expected_q2(i,j) + 1/10*q4(i,j))
...
def plotWaveFunctions(N:int, λ:float, q:str, nlines:int):
    ...
    for i in tqdm(range(nlines), desc = "Plotting"):
        plot, = plt.plot(xs, data[i,:]+3*i - yoffset, zorder = 1)
        plt.axhline(y=3*i- yoffset, color = plot.get_color(), linewidth = 1)
        plt.fill_between(xs, data[i,:]+3*i- yoffset, 3*i- yoffset, color = plot.get_color(), alpha = 0.1)


    plt.plot(xs,(1/2 * np.square(xs) -3/2 * np.power(xs,2) + 1/10 * np.power(xs, 4)), zorder = 0, linestyle = "dashed", color = "gray")
    plt.ylim(- 3, 3 * (nlines+1)- yoffset)
    plt.tick_params(
...
N = 120; λ = 1
eigvals, eigvects = np.linalg.eigh(getNondiagonalHamiltonian(N,1,"[q^4]"))
n = 15
spekter = eigvals[:n]
comparison = [(i+1/2)/2 for i in range(n)]
fig, ax = plt.subplots()
xs = np.linspace(-1/2,n+1/2,n+1)
for i in range(n):
    ax.hlines(spekter[i], xmin=xs[i], xmax=xs[i+1], color=next(ax._get_lines.prop_cycler)['color'] , label = f"Lastno stanje {i}")


#plt.legend()
plt.ylabel("E")
plt.xlabel("Zaporedno št. lastnega stanja")
plt.xticks([i for i in range(0,n+1,1)])
plt.grid()
plt.show()


plotWaveFunctions(N,λ,"[q^4]", 12)
